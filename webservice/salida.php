...
$fecha=date('Y-m-d');
$hora=date('H:i:s');
//http://sitemcode.online/parqueo/webservice/salida.php?lblcode=mpsb125
if (count($existe)>0) 
{
	...
	if (count($traasig)>0)
	{
		  ...
		  //$esta=$estacionamiento->mostrarPrimero('idcategoria='.$trans['idcategoria']." and estadolugar=2");
		  //$idestacionamiento=$esta['idestacionamiento'];
		  //if (count($esta)>0)
		  //	{
		  		//VERIFICAR QUE EXISTA REGISTRO DE INGRESO
		        $moviexist=$movimiento->mostrarUltimo("idtarjeta=".$idtarjeta." and control=1 and estado=1");
		        $idmovimiento=$moviexist['idmovimiento'];
		        if (count($moviexist)>0) 
		        {
		        	// PREGUNTAR SI NO EXISTE MARCADO DE 
		        	$moviexist2=$movimiento->mostrarUltimo("idtarjeta=".$idtarjeta." and control=2 and estado=1 and idmovimiento=".$idmovimiento); 
			  		if (count($moviexist2)>0) 
			  		{
			  			echo 'YA SE HABILITO LA SALIDA';
			  		}else{
							...
							$valores=array(
									...
									"control"=>'2'  //1=Iniciado(solo ingreso) 2=Completado(entrada y salida 
					        );
							if($movimiento->actualizar($valores,$idmovimiento))
							{		
								      $mov=$movimiento->mostrarUltimo($idmovimiento);
										$valoresta=array(
											"estadolugar"=>'0' //0=libre 2=ocupado
										);
									    $estacionamiento->actualizar($valoresta,$mov['idestacionamiento']);
								echo 'SALIDA REGISTRADO...';
							}else{
								echo 'NO REGISTRADO...';
							}
			  		}
		        }else{
		        	echo 'YA REALIZO EL REGISTRO DE SALIDA';
		        }
